...
Government capitation covers only 57\% of students against an 80\% target, while public universities
 have accumulated Ksh 62 billion in unpaid bills—structural constraints that make student attrition
  ...
  \parencite{KIPPRA2024, DailyNation2024}. Recognizing this challenge, Machakos University College 
  undertook a pioneering study applying Business Intelligence and machine learning to student 
  dropout prediction \parencite{ngemu2015}. 
  
  Using the WEKA toolkit to analyze historical student data, researchers 
  evaluated multiple classification algorithms—including Naïve Bayes, Support Vector Machines, 
  and the J48 decision tree—to identify at-risk students based on predictors such as financial 
  stability (fees payment), entry age, health status, and parental occupation \parencite{ngemu2015}.

While the J48 decision tree achieved the highest prediction accuracy in that context (94.4\%), 
and the resulting prototype provided institutional leaders with a framework for evidence-based 
intervention, the study—and subsequent research in the region—suffers from significant gaps \parencite{ngemu2015}. 
Specifically, current models focus almost exclusively on a binary "dropout vs. persistence"
 outcome \parencite{ngemu2015}. 
 
 There is no mention of students at risk of academic failure or significant delays in 
 graduation, which are critical components of educational efficiency that do not necessarily
  result in total withdrawal. This illustrates a recurring East African pattern: technically feasible models that identify "attrition" (defined as persistent absence 
 for no apparent reason) but fail to translate into embedded, long-term institutional practices that address 
 the full spectrum of student risk, including the silent crisis of graduation delay.
